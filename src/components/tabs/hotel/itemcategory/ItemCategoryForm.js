...
import { CREATE_ITEMCATEGORY } from '../../../../scripts/config/RestEndpoints'
...
function ItemCategoryForm(props) {
  ...
  async function createItemCategory(e) {
    setSubmitting(true)
    e.preventDefault()
    const gdFetchOption = {
  ...
  async function updateItemCategory(e) {
    setSubmitting(true)
    e.preventDefault()
    const gdFetchOption = {
      url: CREATE_ITEMCATEGORY,
  ...
  return (
    <Form onSubmit={(e) => (isUpdate ? updateItemCategory(e) : createItemCategory(e))}>
      <Row>
        ...
        <Col xs="12" className="p-1">
          <Spinner
            loading={submitting}
            loadingText={`${isUpdate ? 'Updating item category' : 'Creating item category'}`}
          >
            <Form.Control
              size="md"
              type="submit"
