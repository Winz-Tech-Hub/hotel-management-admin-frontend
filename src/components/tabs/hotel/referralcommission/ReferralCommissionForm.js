...
function ReferralCommissionForm(props) {
  ...
  async function createReferralCommission(e) {
    setSubmitting(true)
    e.preventDefault()
    const gdFetchOption = {
      url: CREATE_PACKAGE,
  ...
  async function updateReferralCommission(e) {
    setSubmitting(true)
    e.preventDefault()
    const gdFetchOption = {
      url: CREATE_PACKAGE,
  ...
  return (
    <Form onSubmit={(e) => (isUpdate ? updateReferralCommission(e) : createReferralCommission(e))}>
      <Row>
        ...
        <Col xs="12" sm="12" md="6" lg="6" className="p-1">
          <Spinner
            loading={submitting}
            loadingText={`${isUpdate ? 'Updating referral commission' : 'Creating referral commission'}`}
          >
            <Form.Control
              size="md"
              type="submit"
...
export default ReferralCommissionForm
